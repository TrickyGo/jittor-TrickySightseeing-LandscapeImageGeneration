...
def eval(epoch, writer):
    ...
    for i, (ref_B, real_A, photo_id) in enumerate(val_dataloader):
        ...
        if i == 0:
            ...
            np.savetxt(f"{opt.output_path}/images/epoch_{epoch}_sample_patterns.txt", pattern_code)
            writer.add_image('val/image', img.transpose(2,0,1), epoch)

            #edit pattern_code
            patterns_edited = patterns_for_editing.copy()
            for i in range(1, opt.batch_size): #copy previous sample's patten code
                patterns_edited[edit_idx][i] = pattern_code[i-1]

            fake_B, _ = generator(real_A, image=None, patterns_for_editing=patterns_edited)
            img_sample = np.concatenate([ref_B.data, fake_B.data], -2)
            img = save_image(img_sample, f"{opt.output_path}/images/epoch_{epoch}_sample_edit.png", nrow=5)

            pattern_code = patterns_edited[edit_idx].data
            np.savetxt(f"{opt.output_path}/images/epoch_{epoch}_sample_edit_patterns.txt", pattern_code)
            writer.add_image('val/image', img.transpose(2,0,1), epoch)
